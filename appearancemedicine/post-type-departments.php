...
if(is_admin())
{
	function theme_departments_admin_menu()
	{
		$permalinks_page = add_submenu_page('edit.php?post_type=departments', __('Permalink', 'medicenter'), __('Permalink', 'medicenter'), 'manage_options', 'departments_permalink', 'theme_departments_permalink');
	}
	add_action("admin_menu", "theme_departments_admin_menu");
	
	function theme_departments_permalink()
	{
		$message = "";
		if(isset($_POST["action"]) && $_POST["action"]=="save_departments_permalink")
			$message = __("Options saved!", "medicenter");
		$departments_permalink = array(
			"slug" => 'departments',
			"label_singular" => __("Department", 'medicenter'),
			"label_plural" => __("Departments", 'medicenter')
		);
		$departments_permalink = array_merge($departments_permalink, (array)get_option("departments_permalink"));
		
		require_once("admin/admin-page-departments-permalink.php");
	}
}
function theme_departments_init()
{
	...
	if(isset($_POST["action"]) && $_POST["action"]=="save_departments_permalink")
	{
		...
		$departments_permalink = array(
			...
			"label_singular" => (!empty($_POST["label_singular"]) ? $_POST["label_singular"] : __("Department", "medicenter")),
			"label_plural" => (!empty($_POST["label_plural"]) ? $_POST["label_plural"] : __("Departments", "medicenter"))
		);
	...
	$labels = array(
		...
		'add_new' => _x('Add New', $departments_permalink["slug"], 'medicenter'),
		'add_new_item' => sprintf(__('Add New %s' , 'medicenter') , $departments_permalink['label_singular']),
		'edit_item' => sprintf(__('Edit %s', 'medicenter'), $departments_permalink['label_singular']),
		'new_item' => sprintf(__('New %s', 'medicenter'), $departments_permalink['label_singular']),
		'all_items' => sprintf(__('All %s', 'medicenter'), $departments_permalink['label_plural']),
		'view_item' => sprintf(__('View %s', 'medicenter'), $departments_permalink['label_singular']),
		'search_items' => sprintf(__('Search %s', 'medicenter'), $departments_permalink['label_plural']),
		'not_found' =>  sprintf(__('No %s found', 'medicenter'), strtolower($departments_permalink['label_plural'])),
		'not_found_in_trash' => sprintf(__('No %s found in Trash', 'medicenter'), strtolower($departments_permalink['label_plural'])), 
		'parent_item_colon' => '',
