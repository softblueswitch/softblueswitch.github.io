...
if(is_admin())
{
	function theme_doctors_admin_menu()
	{
		$permalinks_page = add_submenu_page('edit.php?post_type=doctors', __('Permalink', 'medicenter'), __('Permalink', 'medicenter'), 'manage_options', 'doctors_permalink', 'theme_doctors_permalink');
	}
	add_action("admin_menu", "theme_doctors_admin_menu");
	
	function theme_doctors_permalink()
	{
		$message = "";
		...
		$doctors_permalink = array(
			"slug" => 'doctors',
			"label_singular" => __("Doctor", 'medicenter'),
			"label_plural" => __("Doctors", 'medicenter')
		);
		$doctors_permalink = array_merge($doctors_permalink, (array)get_option("doctors_permalink"));
		
		require_once("admin/admin-page-doctors-permalink.php");
	}
}
function theme_doctors_init()
{
	...
	if(isset($_POST["action"]) && $_POST["action"]=="save_doctors_permalink")
	{
		...
		$doctors_permalink = array(
			...
			"label_singular" => (!empty($_POST["label_singular"]) ? $_POST["label_singular"] : __("Doctor", "medicenter")),
			"label_plural" => (!empty($_POST["label_plural"]) ? $_POST["label_plural"] : __("Doctors", "medicenter"))
		);
	...
	$labels = array(
		...
		'add_new' => _x('Add New', $doctors_permalink["slug"], 'medicenter'),
		'add_new_item' => sprintf(__('Add New %s' , 'medicenter') , $doctors_permalink['label_singular']),
		'edit_item' => sprintf(__('Edit %s', 'medicenter'), $doctors_permalink['label_singular']),
		'new_item' => sprintf(__('New %s', 'medicenter'), $doctors_permalink['label_singular']),
		'all_items' => sprintf(__('All %s', 'medicenter'), $doctors_permalink['label_plural']),
		'view_item' => sprintf(__('View %s', 'medicenter'), $doctors_permalink['label_singular']),
		'search_items' => sprintf(__('Search %s', 'medicenter'), $doctors_permalink['label_plural']),
		'not_found' =>  sprintf(__('No %s found', 'medicenter'), strtolower($doctors_permalink['label_plural'])),
		'not_found_in_trash' => sprintf(__('No %s found in Trash', 'medicenter'), strtolower($doctors_permalink['label_plural'])), 
		'parent_item_colon' => '',
