...
if(is_admin())
{
	function theme_features_admin_menu()
	{
		$permalinks_page = add_submenu_page('edit.php?post_type=features', __('Permalink', 'medicenter'), __('Permalink', 'medicenter'), 'manage_options', 'features_permalink', 'theme_features_permalink');
	}
	add_action("admin_menu", "theme_features_admin_menu");
	
	function theme_features_permalink()
	{
		$message = "";
		...
		$features_permalink = array(
			"slug" => 'features',
			"label_singular" => __("Feature", 'medicenter'),
			"label_plural" => __("Features", 'medicenter')
		);
		$features_permalink = array_merge($features_permalink, (array)get_option("features_permalink"));
		
		require_once("admin/admin-page-features-permalink.php");
	}
}
function theme_features_init()
{
	...
	if(isset($_POST["action"]) && $_POST["action"]=="save_features_permalink")
	{
		...
		$features_permalink = array(
			...
			"label_singular" => (!empty($_POST["label_singular"]) ? $_POST["label_singular"] : __("Feature", "medicenter")),
			"label_plural" => (!empty($_POST["label_plural"]) ? $_POST["label_plural"] : __("Features", "medicenter"))
		);
	...
	$labels = array(
		...
		'add_new' => _x('Add New', $features_permalink["slug"], 'medicenter'),
		'add_new_item' => sprintf(__('Add New %s' , 'medicenter') , $features_permalink['label_singular']),
		'edit_item' => sprintf(__('Edit %s', 'medicenter'), $features_permalink['label_singular']),
		'new_item' => sprintf(__('New %s', 'medicenter'), $features_permalink['label_singular']),
		'all_items' => sprintf(__('All %s', 'medicenter'), $features_permalink['label_plural']),
		'view_item' => sprintf(__('View %s', 'medicenter'), $features_permalink['label_singular']),
		'search_items' => sprintf(__('Search %s', 'medicenter'), $features_permalink['label_plural']),
		'not_found' =>  sprintf(__('No %s found', 'medicenter'), strtolower($features_permalink['label_plural'])),
		'not_found_in_trash' => sprintf(__('No %s found in Trash', 'medicenter'), strtolower($features_permalink['label_plural'])), 
		'parent_item_colon' => '',
