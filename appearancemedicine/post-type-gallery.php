...
if(is_admin())
{
	function theme_gallery_admin_menu()
	{
		$permalinks_page = add_submenu_page('edit.php?post_type=medicenter_gallery', __('Permalink', 'medicenter'), __('Permalink', 'medicenter'), 'manage_options', 'galleries_permalink', 'theme_gallery_permalink');
	}
	add_action("admin_menu", "theme_gallery_admin_menu");
	
	function theme_gallery_permalink()
	{
		$message = "";
		...
		$galleries_permalink = array(
			...
			"label_singular" => __("Gallery Item", 'medicenter'),
			"label_plural" => __("Gallery Items", 'medicenter')
		);
		$galleries_permalink = array_merge($galleries_permalink, (array)get_option("galleries_permalink"));
		
		require_once("admin/admin-page-galleries-permalink.php");
	}
}
function theme_gallery_init()
{
	...
	if(isset($_POST["action"]) && $_POST["action"]=="save_galleries_permalink")
	{
		...
		$galleries_permalink = array(
			...
			"label_singular" => (!empty($_POST["label_singular"]) ? $_POST["label_singular"] : __("Gallery Item", "medicenter")),
			"label_plural" => (!empty($_POST["label_plural"]) ? $_POST["label_plural"] : __("Gallery Items", "medicenter"))
		);
	...
	$labels = array(
		...
		'add_new' => _x('Add New', $galleries_permalink["slug"], 'medicenter'),
		'add_new_item' => sprintf(__('Add New %s' , 'medicenter') , $galleries_permalink['label_singular']),
		'edit_item' => sprintf(__('Edit %s', 'medicenter'), $galleries_permalink['label_singular']),
		'new_item' => sprintf(__('New %s', 'medicenter'), $galleries_permalink['label_singular']),
		'all_items' => sprintf(__('All %s', 'medicenter'), $galleries_permalink['label_plural']),
		'view_item' => sprintf(__('View %s', 'medicenter'), $galleries_permalink['label_singular']),
		'search_items' => sprintf(__('Search %s', 'medicenter'), $galleries_permalink['label_plural']),
		'not_found' =>  sprintf(__('No %s found', 'medicenter'), strtolower($galleries_permalink['label_plural'])),
		'not_found_in_trash' => sprintf(__('No %s found in Trash', 'medicenter'), strtolower($galleries_permalink['label_plural'])), 
		'parent_item_colon' => '',
